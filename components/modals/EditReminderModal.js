...
import { editReminder } from "../../pages/api/reminderApi";
import { useMutation } from "react-query";
import { queryClient } from "../../pages/_app";

function EditReminderModal({ display, close, reminder }) {
  ...
  const threeDays = `${current.getDate()}/${
    current.getMonth() + 1
  }/${current.getFullYear()}`;

  current.setDate(current.getDate() + 7);
  const oneWeek = `${current.getDate()}/${
    current.getMonth() + 1
  }/${current.getFullYear()}`;
    ...
    >
      <div className="relative  mx-auto mt-4  p-5 border pb-12 lg:w-1/3 md:sm:w-3/5 xs:w-3/4 shadow-lg rounded-lg bg-white">
        ...
        >
          ...
          <form className="w-full">
              ...
              {assignee && (
                <ul className="list-none py-2 bg-[#FFFFFF] shadow-lg shadow-[#18A75D66] rounded-lg p-3 my-2 text-gray-400 text-sm border-1 border-[#00A85A]">
                  ...
                  <li
                    ...
                    onClick={() => setDueDate(`3 Business Days`)}
                  >
                    {" "}
            ...
            <div className="flex justify-between space-x-2">
              <div className="mb-4 w-1/2">
                ...
                <div className=" rounded-lg p-2 py-[15px] my-2 text-[#163828] text-xs border flex justify-between items-center  border-[#87AC9B]">
                  <span>
                    {" "}
                    <span className="">00:00</span>
                    <span className="pl-20">
                      <span className="bg-[#00A85A] text-white rounded-sm p-[2px] text-xs">
                        AM{" "}
                      </span>
                      PM
                    </span>
                ...
                {notify && (
                  <ul className="list-none py-2 bg-[#FFFFFF] shadow-lg shadow-[#18A75D66] rounded-lg p-3 my-2 text-gray-400 text-sm border-1 border-[#00A85A]">
                    ...
                    <li className=" border-gray-200 py-4 hover:cursor-pointer text-[#87AC9B] text-xs hover:text-green-600">
                      <span className="font-bold">At task due time</span>
                    </li>
                    <li className="hover:text-green-600 py-4 hover:cursor-pointer text-[#87AC9B] text-xs">
                      {" "}
                      <span className="font-bold"> 30 mins before</span>
                    </li>
                    <li className="hover:text-green-600 py-4 hover:cursor-pointer text-[#87AC9B] text-xs">
                      {" "}
                      <span className="font-bold"> 1 hour before</span>
                    </li>
                    <li className="hover:text-green-600 py-4 hover:cursor-pointer text-[#87AC9B] text-xs">
                      {" "}
                      <span className="font-bold"> 1 day before</span>
                    </li>
                    <li className="hover:text-green-600 py-4 hover:cursor-pointer  text-xs">
                      {" "}
          ...
          <p className="text-[#00A85A] text-xs font-medium">+ ADD CUSTOMER</p>
